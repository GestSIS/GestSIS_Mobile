...
export default function useExercices() {
  ...
  const sync = async (): Promise<boolean> => {
    ...
    .map((e) => {
      ...
      const inProgressReferenceSapeursIds = new Set(inProgressExercice?.initialSapeurs.map(s => s.sapeur_id));
      const inProgressSapeursIds = new Set(inProgressExercice?.sapeurs.map(s => s.sapeur_id));
      const remoteSapeursIds = new Set(e?.sapeurs.map(s => s.sapeur_id));

      const addedSapeurs = e.sapeurs.filter(s => !inProgressReferenceSapeursIds.has(s.sapeur_id));
      ...
      const addedSapeursToKeep  = addedSapeurs.map(s => {
        ...
        const sapeur = inProgressExercice?.sapeurs.find(e => e.sapeur_id == s.sapeur_id);
        if (sapeur?.absent || sapeur?.excuse_type_id || sapeur?.heures?.length || sapeur?.remplace) {
      ...
      const updatedSapeursToKeep = updatedSapeurs?.map(s => {
        ...
        if (referenceSapeur?.absent != s.absent ||
            referenceSapeur?.excuse_type_id != s.excuse_type_id ||
            referenceSapeur?.remplace != s.remplace ||
            referenceSapeur?.present != s.present ||
            referenceSapeur?.heures.length != s.heures.length) { // TODO: Heure comparaison could be improved
          return s;
        }

        // Si non, alors on garde la version locale
        const remoteSapeur = e.sapeurs?.find(e => e.sapeur_id == s.sapeur_id) || null;
        return remoteSapeur ?? s;
      }) || [];
      
      // Sapeurs supprimés
      // Si sapeur saisie le garder autrement le supprimer
      const removedSapeurs = inProgressExercice?.sapeurs.filter(s => !remoteSapeursIds.has(s.sapeur_id));
