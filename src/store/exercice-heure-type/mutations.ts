...
export enum Mutations {
  ...
  UPDATE_HEURE_EXERCICE_TYPE_LISTE = "UPDATE_HEURE_EXERCICE_TYPE_LISTE",
}

export type MutationsType<S = State> = {
  [Mutations.CLEAR_CACHE](state: S): void;
  [Mutations.UPDATE_HEURE_EXERCICE_TYPE_LISTE](state: S, payload: HeureExerciceType[]): void;
};
...
export const mutations: MutationTree<State> & MutationsType = {
  ...
  [Mutations.UPDATE_HEURE_EXERCICE_TYPE_LISTE](state: State, payload: HeureExerciceType[]) {
    state.liste = payload;
  }
};
