import { Phase } from "@/models/bundle";
...
export enum Mutations {
  ...
  UPDATE_PHASE_LISTE = "UPDATE_PHASE_LISTE",
}

export type MutationsType<S = State> = {
  [Mutations.CLEAR_CACHE](state: S): void;
  [Mutations.UPDATE_PHASE_LISTE](state: S, payload: Phase[]): void;
};
...
export const mutations: MutationTree<State> & MutationsType = {
  ...
  [Mutations.UPDATE_PHASE_LISTE](state: State, payload: Phase[]) {
    state.liste = payload;
  }
};
